 ...
 */
$date = __get_remote_date($argv[1], "GMT", -1);
$date_l = __get_remote_date($argv[1], "GMT", -1, "GMT", true);
	...
	if (in_array($campaignid, array_keys($agents))) {
		...
		if (mysql_num_rows($result) != 0) {
			...
			$sql = sprintf(
				...
				$r_push['clicks'] - $row['raws'], $r_push['uniqueClicks'] - $row['uniques'], $r_push['chargeBacks'] - $row['chargebacks'], $r_push['signups'] - $row['signups'], null, $r_push['sales'] - $row['sales_number'],
				$date_l
			);
		} else {
			$sql = sprintf(
				...
				$r_push['clicks'], $r_push['uniqueClicks'], $r_push['chargeBacks'], $r_push['signups'], null, $r_push['sales'],
				$date_l
			);
		}
